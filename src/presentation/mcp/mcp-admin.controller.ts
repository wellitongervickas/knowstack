...
import {
  IMcpServerFactory,
  MCP_ADMIN_SERVER_FACTORY,
} from '@/core/interfaces/mcp/mcp-server.interface';
import {
  IStructuredLogger,
  STRUCTURED_LOGGER,
...
/**
 * MCP Admin Controller — organization and project management.
 *
 * No ConfigTenantMiddleware — these tools operate before tenant context exists.
 * No authentication — local-first mode.
 */
@Controller('mcp/admin')
export class McpAdminController {
  constructor(
    @Inject(MCP_ADMIN_SERVER_FACTORY)
  ...
  private async handleMcpRequest(req: Request, res: Response): Promise<void> {
    ...
    this.logger.info('MCP admin request started', {
      method: req.method,
      path: req.path,
    });
    ...
    try {
      ...
      this.logger.error('MCP admin request failed', error instanceof Error ? error : undefined, {
        method: req.method,
        path: req.path,
      ...
      this.logger.info('MCP admin request completed', {
        method: req.method,
        path: req.path,
        latencyMs,
      });
